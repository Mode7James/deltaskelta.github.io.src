...
updatedAt: 2019-07-21T15:19:21.253Z
images:
...
- An edge list is just a list of all of the edges in teh graph. $ edges = (A, B), (A, C),(A, D)... $.
...
runs in $ O(|E|) $ and another runs in $ O(|V|^{3/2}) $ it will depend on the number of
vertices and edges in a particular graph to see which one actually runs faster.

- dense graphs are when the $ |E| \approx |V|^2 $. (each vertex has many edges)
- sparse graphs are when $ |E| \approx |V| $. (each vertex has only a few edges)
...
runtime of this algorithm is $ O(|V| + |E|) $ because in the worst case it will visit each vertex and
